...
exports.handler = async function handler(event) {
  ...
  if (!message) {
    return json(400, { ok: false, error: "Mensagem vazia." });
  }

  if (!apiKey) {
    return json(400, { ok: false, error: "NVIDIA_API_KEY ausente no backend." });
  }
  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      return json(upstream.status, { ok: false, error: raw || `Erro HTTP ${upstream.status}` });
    }

    const data = await upstream.json();
    const answer = extractAssistantText(data);
    if (!answer) {
      return json(502, { ok: false, error: "Resposta vazia da NVIDIA." });
    }
    ...
    if (error && error.name === "AbortError") {
      return json(504, { ok: false, error: "Tempo limite ao consultar a NVIDIA." });
    }
    return json(500, { ok: false, error: String(error?.message || error || "Falha interna.") });
  } finally {
